...
/* Bibliothek fÃ¼r Evolventen-ZahnrÃ¤der, Schnecken und Zahnstangen
...
Beispiele fÃ¼r jedes Modul befinden sich auskommentiert am Ende dieser Datei

Autor:		Dr JÃ¶rg Janssen
...
pi = 3.14159;
rad = 57.29578;
spiel = clearance;	// Spiel zwischen ZÃ¤hnen
...
/*  Kugelevolventen-Funktion
    ...
    theta0 = Polarwinkel des Kegels, an dessen Schnittkante zur GroÃŸkugel die Evolvente abrollt
    theta = Polarwinkel, fÃ¼r den der Azimutwinkel der Evolvente berechnet werden soll */
...
/*	grÃ¶ÃŸter gemeinsamer Teiler
	...
	Sortierung: a muss grÃ¶ÃŸer als b sein */
function ggt(a,b) = 
	a%b == 0 ? b : ggt(b,a%b);
...
/*	Kopiert und dreht einen KÃ¶rper */
...
/*  Zahnstange
    modul = HÃ¶he des Zahnkopfes Ã¼ber der WÃ¤lzgeraden
    laenge = LÃ¤nge der Zahnstange
    hoehe = HÃ¶he der Zahnstange bis zur WÃ¤lzgeraden
    breite = Breite eines Zahns
    eingriffswinkel = Eingriffswinkel, Standardwert = 20Â° gemÃ¤ÃŸ DIN 867. Sollte nicht grÃ¶ÃŸer als 45Â° sein.
    schraegungswinkel = SchrÃ¤gungswinkel zur Zahnstangen-Querachse; 0Â° = Geradverzahnung */
module zahnstange(modul, laenge, hoehe, breite, eingriffswinkel = 20, schraegungswinkel = 0) {

	// Dimensions-Berechnungen
	modul=modul*(1-spiel);
	c = modul / 6;												// Kopfspiel
	mx = modul/cos(schraegungswinkel);							// Durch SchrÃ¤gungswinkel verzerrtes modul in x-Richtung
	a = 2*mx*tan(eingriffswinkel)+c*tan(eingriffswinkel);		// Flankenbreite
	b = pi*mx/2-2*mx*tan(eingriffswinkel);						// Kopfbreite
	x = breite*tan(schraegungswinkel);							// Verschiebung der Oberseite in x-Richtung durch SchrÃ¤gungswinkel
	nz = ceil((laenge+abs(2*x))/(pi*mx));						// Anzahl der ZÃ¤hne
...
/*  Stirnrad
    modul = HÃ¶he des Zahnkopfes Ã¼ber dem Teilkreis
    zahnzahl = Anzahl der RadzÃ¤hne
    breite = Zahnbreite
    bohrung = Durchmesser der Mittelbohrung
    eingriffswinkel = Eingriffswinkel, Standardwert = 20Â° gemÃ¤ÃŸ DIN 867. Sollte nicht grÃ¶ÃŸer als 45Â° sein.
    schraegungswinkel = SchrÃ¤gungswinkel zur Rotationsachse; 0Â° = Geradverzahnung
	optimiert = LÃ¶cher zur Material-/Gewichtsersparnis bzw. OberflÃ¤chenvergÃ¶ÃŸerung erzeugen, wenn Geometrie erlaubt */
module stirnrad(modul, zahnzahl, breite, bohrung, nabendicke=0, nabendurchmesser=0, eingriffswinkel = 20, schraegungswinkel = 0, optimiert = true) {
	...
	alpha_stirn = atan(tan(eingriffswinkel)/cos(schraegungswinkel));// SchrÃ¤gungswinkel im Stirnschnitt
	...
	df = d - 2 * (modul + c);										// FuÃŸkreisdurchmesser
	rf = df / 2;													// FuÃŸkreisradius
	...
	gamma = rad*breite/(r*tan(90-schraegungswinkel));				// Torsionswinkel fÃ¼r Extrusion
	schritt = rho_ra/16;											// Evolvente wird in 16 StÃ¼cke geteilt
	tau = 360/zahnzahl;												// Teilungswinkel
	
	r_loch = (2*rf - bohrung)/8;									// Radius der LÃ¶cher fÃ¼r Material-/Gewichtsersparnis
	rm = bohrung/2+2*r_loch;										// Abstand der Achsen der LÃ¶cher von der Hauptachse
	z_loch = floor(2*pi*rm/(3*r_loch));								// Anzahl der LÃ¶cher fÃ¼r Material-/Gewichtsersparnis
	...
	union(){
		rotate([0,0,-phi_r-90*(1-spiel)/zahnzahl]){						// Zahn auf x-Achse zentrieren;
																		// macht Ausrichtung mit anderen RÃ¤dern einfacher
...
/*  Pfeilrad; verwendet das Modul "stirnrad"
    ...
    bohrung = Durchmesser der Mittelbohrung
    eingriffswinkel = Eingriffswinkel, Standardwert = 20Â° gemÃ¤ÃŸ DIN 867. Sollte nicht grÃ¶ÃŸer als 45Â° sein.
    schraegungswinkel = SchrÃ¤gungswinkel zur Rotationsachse, Standardwert = 0Â° (Geradverzahnung)
	optimiert = LÃ¶cher zur Material-/Gewichtsersparnis */
module pfeilrad(modul, zahnzahl, breite, bohrung, nabendicke, nabendurchmesser, eingriffswinkel = 20, schraegungswinkel=0, optimiert=true){
	...
	translate([0,0,breite]){
		union(){
			stirnrad(modul, zahnzahl, breite, 2*(rm+r_loch*1.49), nabendicke, nabendurchmesser, eingriffswinkel, schraegungswinkel, false);		// untere HÃ¤lfte
			mirror([0,0,1]){
				stirnrad(modul, zahnzahl, breite, 2*(rm+r_loch*1.49), nabendicke, nabendurchmesser, eingriffswinkel, schraegungswinkel, false);	// obere HÃ¤lfte
			}
		}
	}
	// mit Materialersparnis
...
/*	Zahnstange und -Rad
    ...
    zahnzahl_rad = Anzahl der RadzÃ¤hne
	hoehe_stange = HÃ¶he der Zahnstange bis zur WÃ¤lzgeraden
...
/*	Hohlrad
    ...
    zahnzahl = Anzahl der RadzÃ¤hne
    breite = Zahnbreite
	randbreite = Breite des Randes ab FuÃŸkreis
    bohrung = Durchmesser der Mittelbohrung
    eingriffswinkel = Eingriffswinkel, Standardwert = 20Â° gemÃ¤ÃŸ DIN 867. Sollte nicht grÃ¶ÃŸer als 45Â° sein.
    schraegungswinkel = SchrÃ¤gungswinkel zur Rotationsachse, Standardwert = 0Â° (Geradverzahnung) */
module hohlrad(modul, zahnzahl, breite, randbreite, eingriffswinkel = 20, schraegungswinkel = 0) {
	...
	// Dimensions-Berechnungen	
	ha = (zahnzahl >= 20) ? 0.02 * atan((zahnzahl/15)/pi) : 0.6;	// VerkÃ¼rzungsfaktor ZahnkopfhÃ¶he
	d = modul * zahnzahl;											// Teilkreisdurchmesser
	r = d / 2;														// Teilkreisradius
	alpha_stirn = atan(tan(eingriffswinkel)/cos(schraegungswinkel));// SchrÃ¤gungswinkel im Stirnschnitt
	db = d * cos(alpha_stirn);										// Grundkreisdurchmesser
	...
	ra = da / 2;													// Kopfkreisradius
	df = d - 2 * modul * ha;										// FuÃŸkreisdurchmesser
	rf = df / 2;													// FuÃŸkreisradius
	...
	linear_extrude(height = breite, twist = gamma){
		difference(){
			circle(r = ra + randbreite);							// AuÃŸenkreis
			union(){
				zahnbreite = (180*(1+spiel))/zahnzahl+2*phi_r;
				circle(rf);											// FuÃŸkreis	
				for (rot = [0:tau:360]){
	...
	echo("AuÃŸendurchmesser Hohlrad = ", 2*(ra + randbreite));
	
}

/*  Pfeil-Hohlrad; verwendet das Modul "hohlrad"
    modul = HÃ¶he des Zahnkopfes Ã¼ber dem Teilkegel
    zahnzahl = Anzahl der RadzÃ¤hne
    breite = Zahnbreite
    bohrung = Durchmesser der Mittelbohrung
    eingriffswinkel = Eingriffswinkel, Standardwert = 20Â° gemÃ¤ÃŸ DIN 867. Sollte nicht grÃ¶ÃŸer als 45Â° sein.
...
module pfeilhohlrad(modul, zahnzahl, breite, randbreite, eingriffswinkel = 20, schraegungswinkel = 0) {
	...
	translate([0,0,breite])
		union(){
		hohlrad(modul, zahnzahl, breite, randbreite, eingriffswinkel, schraegungswinkel);		// untere HÃ¤lfte
		mirror([0,0,1])
			hohlrad(modul, zahnzahl, breite, randbreite, eingriffswinkel, schraegungswinkel);	// obere HÃ¤lfte
	}
}

/*	Planetengetriebe; verwendet die Module "pfeilrad" und "pfeilhohlrad"
    modul = HÃ¶he des Zahnkopfes Ã¼ber dem Teilkegel
    zahnzahl_sonne = Anzahl der ZÃ¤hne des Sonnenrads
    zahnzahl_planet = Anzahl der ZÃ¤hne eines Planetenrads
    anzahl_planeten = Anzahl der PlanetenrÃ¤der. Wenn null, rechnet die Funktion die Mindestanzahl aus.
    breite = Zahnbreite
	randbreite = Breite des Randes ab FuÃŸkreis
    bohrung = Durchmesser der Mittelbohrung
    eingriffswinkel = Eingriffswinkel, Standardwert = 20Â° gemÃ¤ÃŸ DIN 867. Sollte nicht grÃ¶ÃŸer als 45Â° sein.
	...
	zusammen_gebaut = 
	optimiert = LÃ¶cher zur Material-/Gewichtsersparnis bzw. OberflÃ¤chenvergÃ¶ÃŸerung erzeugen, wenn Geometrie erlaubt
	zusammen_gebaut = Komponenten zusammengebaut fÃ¼r Konstruktion oder auseinander zum 3D-Druck	*/
module planetengetriebe(modul, zahnzahl_sonne, zahnzahl_planet, anzahl_planeten, breite, randbreite, bohrung, eingriffswinkel=20, schraegungswinkel=0, zusammen_gebaut=true, optimiert=true){
	...
	zahnzahl_hohlrad = zahnzahl_sonne + 2*zahnzahl_planet;				// Anzahl der ZÃ¤hne des Hohlrades
	...
	n_max = floor(180/asin(modul*(zahnzahl_planet)/(modul*(zahnzahl_sonne +  zahnzahl_planet))));
																		// Anzahl PlanetenrÃ¤der: hÃ¶chstens so viele, wie ohne
																		// Ãœberlappung mÃ¶glich
	...
	if (zusammen_gebaut){
        if(anzahl_planeten==0){
            ...
            anzahl_planeten = list[0];										// Ermittele Anzahl PlanetenrÃ¤der
            ...
            for(n=[0:1:anzahl_planeten-1]){
                translate(kugel_zu_kart([achsabstand,90,360/anzahl_planeten*n]))
					rotate([0,0,n*360*d_sonne/d_planet])
						pfeilrad (modul, zahnzahl_planet, breite, bohrung, nabendicke=0, nabendurchmesser=0, eingriffswinkel=eingriffswinkel, schraegungswinkel=schraegungswinkel);	// PlanetenrÃ¤der
            }
       }
       else{
            achsabstand = modul*(zahnzahl_sonne + zahnzahl_planet)/2;		// Abstand von Sonnenrad-/Hohlradachse
...
/*  Kegelrad
    modul = HÃ¶he des Zahnkopfes Ã¼ber dem Teilkegel; Angabe fÃ¼r die Aussenseite des Kegels
    zahnzahl = Anzahl der RadzÃ¤hne
    teilkegelwinkel = (Halb)winkel des Kegels, auf dem das jeweils andere Hohlrad abrollt
    zahnbreite = Breite der ZÃ¤hne von der AuÃŸenseite in Richtung Kegelspitze
    bohrung = Durchmesser der Mittelbohrung
    eingriffswinkel = Eingriffswinkel, Standardwert = 20Â° gemÃ¤ÃŸ DIN 867. Sollte nicht grÃ¶ÃŸer als 45Â° sein.
	schraegungswinkel = SchrÃ¤gungswinkel, Standardwert = 0Â° */
module kegelrad(modul, zahnzahl, teilkegelwinkel, zahnbreite, bohrung, eingriffswinkel = 20, schraegungswinkel=0) {

	// Dimensions-Berechnungen
	d_aussen = modul * zahnzahl;									// Teilkegeldurchmesser auf der KegelgrundflÃ¤che,
																	// entspricht der Sehne im Kugelschnitt
	r_aussen = d_aussen / 2;										// Teilkegelradius auf der KegelgrundflÃ¤che 
	rg_aussen = r_aussen/sin(teilkegelwinkel);						// GroÃŸkegelradius fÃ¼r Zahn-AuÃŸenseite, entspricht der LÃ¤nge der Kegelflanke;
	rg_innen = rg_aussen - zahnbreite;								// GroÃŸkegelradius fÃ¼r Zahn-Innenseite	
	...
	rkf = rg_aussen*sin(delta_f);									// Radius des KegelfuÃŸes
	hoehe_f = rg_aussen*cos(delta_f);								// HÃ¶he des Kegels vom FuÃŸkegel
	
	echo("Teilkegeldurchmesser auf der KegelgrundflÃ¤che = ", d_aussen);
	
	// GrÃ¶ÃŸen fÃ¼r KomplementÃ¤r-Kegelstumpf
	hoehe_k = (rg_aussen-zahnbreite)/cos(teilkegelwinkel);			// HÃ¶he des KomplementÃ¤rkegels fÃ¼r richtige ZahnlÃ¤nge
	rk = (rg_aussen-zahnbreite)/sin(teilkegelwinkel);				// FuÃŸradius des KomplementÃ¤rkegels
	rfk = rk*hoehe_k*tan(delta_f)/(rk+hoehe_k*tan(delta_f));		// Kopfradius des Zylinders fÃ¼r 
																	// KomplementÃ¤r-Kegelstumpf
	hoehe_fk = rk*hoehe_k/(hoehe_k*tan(delta_f)+rk);				// Hoehe des KomplementÃ¤r-Kegelstumpfs

	echo("HÃ¶he Kegelrad = ", hoehe_f-hoehe_fk);
	
	phi_r = kugelev(delta_b, teilkegelwinkel);						// Winkel zum Punkt der Evolvente auf Teilkegel
		
	// Torsionswinkel gamma aus SchrÃ¤gungswinkel
		...
		translate([0,0,hoehe_f]) rotate(a=[0,180,0]){
			union(){
				translate([0,0,hoehe_f]) rotate(a=[0,180,0]){								// Kegelstumpf							
					difference(){
						linear_extrude(height=hoehe_f-hoehe_fk, scale=rfk/rkf) circle(rkf*1.001); // 1 promille Ãœberlappung mit ZahnfuÃŸ
						translate([0,0,-1]){
					...
					rotate (rot) {															// "Zahnzahl-mal" kopieren und drehen
						union(){
							if (delta_b > delta_f){
								...
								polyhedron(
									points = [
										kugel_zu_kart([rg_aussen, start*1.001, flankenpunkt_unten]),	// 1 promille Ãœberlappung mit Zahn
...
module pfeilkegelrad(modul, zahnzahl, teilkegelwinkel, zahnbreite, bohrung, eingriffswinkel = 20, schraegungswinkel=0){
	...
	d_aussen = modul * zahnzahl;								// Teilkegeldurchmesser auf der KegelgrundflÃ¤che,
																// entspricht der Sehne im Kugelschnitt
	r_aussen = d_aussen / 2;									// Teilkegelradius auf der KegelgrundflÃ¤che 
	rg_aussen = r_aussen/sin(teilkegelwinkel);					// GroÃŸkegelradius, entspricht der LÃ¤nge der Kegelflanke;
	c = modul / 6;												// Kopfspiel
	df_aussen = d_aussen - (modul +c) * 2 * cos(teilkegelwinkel);
	rf_aussen = df_aussen / 2;
	delta_f = asin(rf_aussen/rg_aussen);
	...
	modul_innen = modul*(1-zahnbreite/rg_aussen);
		...
		union(){
		kegelrad(modul, zahnzahl, teilkegelwinkel, zahnbreite, bohrung, eingriffswinkel, schraegungswinkel);		// untere HÃ¤lfte
		translate([0,0,hoehe_f-hoehe_fk])
			rotate(a=-gamma,v=[0,0,1])
				kegelrad(modul_innen, zahnzahl, teilkegelwinkel, zahnbreite, bohrung, eingriffswinkel, -schraegungswinkel);	// obere HÃ¤lfte
	}
}
	...
	for(i=[0:1:schritte-1]){
		...
		translate([0,0,b*cos(teilkegelwinkel)*i])
			...
			rotate(a=-schraegungswinkel*i,v=[0,0,1])
				kegelrad(modul_r, zahnzahl, teilkegelwinkel, b, bohrung, eingriffswinkel, schraegungswinkel);	// obere HÃ¤lfte
		}
	}
}
...
/*	Kegelradpaar mit beliebigem Achsenwinkel; verwendet das Modul "kegelrad"
    modul = HÃ¶he des Zahnkopfes Ã¼ber dem Teilkegel; Angabe fÃ¼r die Aussenseite des Kegels
    zahnzahl_rad = Anzahl der RadzÃ¤hne am Rad
    zahnzahl_ritzel = Anzahl der RadzÃ¤hne am Ritzel
...
module kegelradpaar(modul, zahnzahl_rad, zahnzahl_ritzel, achsenwinkel=90, zahnbreite, bohrung_rad, bohrung_ritzel, eingriffswinkel=20, schraegungswinkel=0, zusammen_gebaut=true){
	...
	rg = r_rad/sin(delta_rad);								// Radius der GroÃŸkugel
	c = modul / 6;											// Kopfspiel
	df_ritzel = pi*rg*delta_ritzel/90 - 2 * (modul + c);	// FuÃŸkegeldurchmesser auf der GroÃŸkugel 
	rf_ritzel = df_ritzel / 2;								// FuÃŸkegelradius auf der GroÃŸkugel
	delta_f_ritzel = rf_ritzel/(pi*rg) * 180;				// Kopfkegelwinkel
	rkf_ritzel = rg*sin(delta_f_ritzel);					// Radius des KegelfuÃŸes
	hoehe_f_ritzel = rg*cos(delta_f_ritzel);				// HÃ¶he des Kegels vom FuÃŸkegel
	
	echo("Kegelwinkel Rad = ", delta_rad);
	echo("Kegelwinkel Ritzel = ", delta_ritzel);
 
	df_rad = pi*rg*delta_rad/90 - 2 * (modul + c);			// FuÃŸkegeldurchmesser auf der GroÃŸkugel 
	rf_rad = df_rad / 2;									// FuÃŸkegelradius auf der GroÃŸkugel
	delta_f_rad = rf_rad/(pi*rg) * 180;						// Kopfkegelwinkel
	rkf_rad = rg*sin(delta_f_rad);							// Radius des KegelfuÃŸes
	hoehe_f_rad = rg*cos(delta_f_rad);						// HÃ¶he des Kegels vom FuÃŸkegel

	echo("HÃ¶he Rad = ", hoehe_f_rad);
	echo("HÃ¶he Ritzel = ", hoehe_f_ritzel);
...
modul = HÃ¶he des Schneckenkopfes Ã¼ber dem Teilzylinder
gangzahl = Anzahl der GÃ¤nge (ZÃ¤hne) der Schnecke
laenge = LÃ¤nge der Schnecke
bohrung = Durchmesser der Mittelbohrung
eingriffswinkel = Eingriffswinkel, Standardwert = 20Â° gemÃ¤ÃŸ DIN 867. Sollte nicht grÃ¶ÃŸer als 45Â° sein.
steigungswinkel = Steigungswinkel der Schnecke, entspricht 90Â° minus SchrÃ¤gungswinkel. Positiver Steigungswinkel = rechtsdrehend.
zusammen_gebaut = Komponenten zusammengebaut fÃ¼r Konstruktion oder auseinander zum 3D-Druck */
module schnecke(modul, gangzahl, laenge, bohrung, eingriffswinkel=20, steigungswinkel, zusammen_gebaut=true){
	...
	rf = r - modul - c;											// FuÃŸzylinder-Radius
	a = modul*gangzahl/(90*tan(eingriffswinkel));				// Spiralparameter
	tau_max = 180/gangzahl*tan(eingriffswinkel);				// Winkel von FuÃŸ zu Kopf in der Normalen
	gamma = -rad*laenge/((rf+modul+c)*tan(steigungswinkel));	// Torsionswinkel fÃ¼r Extrusion
...
/*
Berechnet einen Schneckenradsatz. Das Schneckenrad ist ein gewÃ¶hnliches Stirnrad ohne Globoidgeometrie.
modul = HÃ¶he des Schneckenkopfes Ã¼ber dem Teilzylinder bzw. des Zahnkopfes Ã¼ber dem Teilkreis
zahnzahl = Anzahl der RadzÃ¤hne
gangzahl = Anzahl der GÃ¤nge (ZÃ¤hne) der Schnecke
breite = Zahnbreite
laenge = LÃ¤nge der Schnecke
...
bohrung_rad = Durchmesser der Mittelbohrung des Stirnrads
eingriffswinkel = Eingriffswinkel, Standardwert = 20Â° gemÃ¤ÃŸ DIN 867. Sollte nicht grÃ¶ÃŸer als 45Â° sein.
steigungswinkel = Steigungswinkel der Schnecke, entspricht 90Â°-SchrÃ¤gungswinkel. Positiver Steigungswinkel = rechtsdrehend.
optimiert = LÃ¶cher zur Material-/Gewichtsersparnis
zusammen_gebaut =  Komponenten zusammengebaut fÃ¼r Konstruktion oder auseinander zum 3D-Druck */
module schneckenradsatz(modul, zahnzahl, gangzahl, breite, laenge, bohrung_schnecke, bohrung_rad, nabendurchmesser, nabendicke, eingriffswinkel=20, steigungswinkel, optimiert=true, zusammen_gebaut=true){
	...
	rf_schnecke = r_schnecke - modul - c;						// FuÃŸzylinder-Radius
